    ...
    <nav class="navbar navbar-inverse navbar-fixed-top">
        ...
        echo"   <div class='container-fluid col-sm-12 col-md-12 col-lg-12 col-xl-12 panel panel-primary' style='margin-top: 100px;margin-left: 20px'>
                    <div class='row' style='margin-left: 20px;margin-bottom: 20px'>
                        <h3>PENDAFTARAN AKTA KEMATIAN</h3>
                        <h6>silahkan masukkan berkas</h6>
                        
                        <form method='post' enctype='multipart/form-data'>
                            
                                <label>SURAT KETERANGAN KEMATIAN DARI DESA</label><br>
                                ...
                                <label>KETERANGAN KEMATIAN DARI DOKTER ATAU PRAMEDIS ASLI</label></br>
                                <label id='update_profile'><i class='fa fa-circle-o' aria-hidden='true'></i></br>
                                <input type='file' name='u_image4' size='60'></br>
                                </label></br>

                                <label>KTP YANG MENINGGAL (Sesuai Ketentuan)</label></br>
                                <label id='update_profile'><i class='fa fa-circle-o' aria-hidden='true'></i></br>
                                <input type='file' name='u_image5' size='60'></br>
                                </label></br>

                                <label>KTP SUAMI/ISTRI DARI YANG MENINGGAL</label></br>
                                <label id='update_profile'><i class='fa fa-circle-o' aria-hidden='true'></i></br>
                                <input type='file' name='u_image6' size='60'></br>
                                </label></br>

                                <label>SCAN SURAT KUASA MATERAI 6000</label></br>
                                <label id='update_profile'><i class='fa fa-circle-o' aria-hidden='true'></i></br>
        ...
        if (isset($_POST['update'])) {
            ...
            if ($u_image1=='') {
                ...
                move_uploaded_file($image_tmp1, "../files/Surat_Kematian_Desa/$u_image1");
                move_uploaded_file($image_tmp2, "../files/KTP_Pelapor/$u_image2");
                move_uploaded_file($image_tmp3, "../files/KK_Asli/$u_image3");
                move_uploaded_file($image_tmp4, "../files/Surat_kematian_dokter/$u_image4");
                move_uploaded_file($image_tmp5, "../files/ktp_asli_meninggal/$u_image5");
                move_uploaded_file($image_tmp6, "../files/ktp_pasangan/$u_image6");
                move_uploaded_file($image_tmp7, "../files/surat_kuasa/$u_image7");
                               
                
                $insert_track = "INSERT INTO perjalanan (user_nik, nama_data, daftar, upload, proses, selesai) VALUES ('$user_nik', 'Akta Kematian', 'Terdaftar', 'Telah Diupload', 'Masih Dalam Proses', 'Belum')";
                $run = mysqli_query($con, $insert_track);
                $insert = "INSERT INTO data (user_nik, nama_data, file_1, file_2, file_3, file_4, file_5, file_6, file_7, file_8, file_9, file_10, file_11, file_12, file_13, file_14) VALUES ('$user_nik', 'Akta Kematian', '/files/Surat_Kematian_Desa/$u_image1', '/files/KTP_Pelapor/$u_image2', '/files/KK_Asli/$u_image3', '/files/Surat_kematian_dokter/$u_image4', '/files/ktp_asli_meninggal/$u_image5', '/files/ktp_pasangan/$u_image6', '/files/surat_kuasa/$u_image7', '-', '-', '-', '-', '-', '-', '-')";
                $run = mysqli_query($con, $insert);
    
                if ($run) {
                    echo "<script>alert('Data Telah diupload')</script>";
                    echo "<script>window.open('akta_kematian.php', '_self')</script>";
                }
            }
        }
        ?>
