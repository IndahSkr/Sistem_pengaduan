    ...
    <nav class="navbar navbar-inverse navbar-fixed-top">
        ...
        echo"   <div class='container-fluid col-sm-12 col-md-12 col-lg-12 col-xl-12 panel panel-primary' style='margin-top: 100px;margin-left: 20px'>
                    <div class='row' style='margin-left: 20px;margin-bottom: 20px'>
                        <h3>PENDAFTARAN KIA (KARTU IDENTITAS ANAK)</h3>
        ...
        if (isset($_POST['update'])) {
            ...
            if ($u_image1=='') {
                ...
                move_uploaded_file($image_tmp1, "../files/kk_asli/$u_image1");
                move_uploaded_file($image_tmp2, "../files/akta_lahir/$u_image2");
                                             
                
                $insert_track = "INSERT INTO perjalanan (user_nik, nama_data, daftar, upload, proses, selesai) VALUES ('$user_nik', 'KIA', 'Terdaftar', 'Telah Diupload', 'Masih Dalam Proses', 'Belum')";
                $run = mysqli_query($con, $insert_track);
                $insert = "INSERT INTO data (user_nik, nama_data, file_1, file_2, file_3, file_4, file_5, file_6, file_7, file_8, file_9, file_10, file_11, file_12, file_13, file_14) VALUES ('$user_nik', 'KIA', '/files/kk_asli/$u_image1', '/files/akta_lahir/$u_image2', '-', '-', '-', '-', '-', '-', '-', '-', '-', '-', '-', '-')";
                $run = mysqli_query($con, $insert);
    
                if ($run) {
                    echo "<script>alert('Data Telah diupload')</script>";
                    echo "<script>window.open('kia.php', '_self')</script>";
                }
            }
        }
        ?>
